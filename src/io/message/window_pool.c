...
static mc_msg_idx get_index(const wndpool_t* const this, const mc_msg_id id)
{
...
static wnd_t* get_window(const wndpool_t* const this, const mc_msg_idx index)
{
...
void wndpool_clear(wndpool_t* const this)
{
  ...
  for (mc_msg_idx index = 0; index < this->capacity; index++) {
...
bool wndpool_contains(wndpool_t* const this, mc_msg_id id)
{
  return ((this->bgn_id <= id) && (id < this->bgn_id + this->capacity));
}

wnd_t* wndpool_get(wndpool_t* const this, mc_msg_id id)
{
...
bool wndpool_insert(wndpool_t* const this, const mc_span data, const mc_msg_id id)
{
  if (!wndpool_contains(this, id)) {
    return false;
  }
  
  const mc_msg_idx index = get_index(this, id);
...
bool wndpool_ack(wndpool_t* const this, mc_msg_id id, mc_io_receive_cb on_done)
{
  if (!wndpool_contains(this, id)) {
    return false;
  }
